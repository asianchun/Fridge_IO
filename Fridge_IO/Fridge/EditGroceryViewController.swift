...
class EditGroceryViewController: UIViewController {
    ...
    //Create the action separately, so that we can set the active choice for the grocery
    //i.e. the edit page will display the selected grocery's type as an already selected option
    func createAction(_ name: String, handler: @escaping UIActionHandler) -> UIAction {
        ...
        //Check whether the current grocery is of that type, if so display it as already selected
